...
{
 // обновление состо€ни€ команды. источник закрываетс€, если
 // он открыт. анализируетс€ IsInitialized
...
void __fastcall TmainForm::OpenVideoActionExecute(TObject *Sender)
{
    ...
    if (OpenDialog1->Execute())
    {
        ...
        {
            ShowMessage(L"Ќе могу открыть файл: " + OpenDialog1->FileName);
            return;
        }
...
void __fastcall TmainForm::FormShow(TObject *Sender)
{
	...
	Caption = L"ќбнаружение и распознавание номеров вагонов.  v " + vi->FileVersion;
    delete vi;
}
//---------------------------------------------------------------------------
...
void __fastcall TmainForm::GotoFrameActionExecute(TObject *Sender)
{
  ...
  UnicodeString InputString = InputBox(L"Trains demo", L"ѕереход на кадр", IntToStr(FImage1->Frames->CurrentFrame));
  if (InputString != IntToStr(FImage1->Frames->CurrentFrame))
  {
  	 try
     {
        ...
        {
        	ShowMessage("Ќедопустимый номер кадра.");
            return;
        }
          ...
          BYTE *data)
{
    if (m_videoSource != NULL)
    {
		StatusBar1->Panels->Items[1]->Text = L" адр "  + IntToStr( m_videoSource->CurrentFrame) +
   		" of " + IntToStr(m_videoSource->NumFrames);
	    StatusBar1->Panels->Items[3]->Text = L"»зображение: " + IntToStr(w) + L":" + IntToStr(h) + L":" + IntToStr(c);
    }
    else
...
void __fastcall TmainForm::SetSource(TPhMediaSource* source)
{
   ...
   if (!this->InitParams())
   {
      delete m_videoSource;
      m_videoSource = NULL;
      ShowMessage("Ќевозможно инициализировать параметры!");
      return;
   }
   ...
   if (m_videoSource  != NULL)
   {
		...
		StatusBar1->Panels->Items[1]->Text = L" адр "  + IntToStr( m_videoSource->CurrentFrame) +
   		" of " + IntToStr(m_videoSource->NumFrames);
	   StatusBar1->Panels->Items[2]->Text = L"»сточник данных: " + m_videoSource->SourceName;
       //StatusBar1->Panels->Items[3]->Text = L"»зображение: " + IntToStr(w) + L":" + IntToStr(h) + L":" + IntToStr(c);
       UnicodeString cap = L"ќбнаружение и распознавание номеров вагонов.  [";
   ...
   {
	   StatusBar1->Panels->Items[1]->Text = L"";
	   StatusBar1->Panels->Items[2]->Text = L"»сточник данных: NULL";
       this->Caption = L"ќбнаружение и распознавание номеров вагонов.";
...
void __fastcall TmainForm::SetMode(TAction* action)
{
    ...
    {
        FImage1->SelectPhTool(PhPaneTool1);
        StatusBar1->Panels->Items[0]->Text = L"–ежим: ";
