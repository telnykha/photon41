...
bool minRegularTetrahedronAroundPointsNR ( CArrRef<Vector3d> data, Polyhedron & poly )
{
    ...
    const double c = sqrt ( 0.75 );
    poly.makeTetrahedron(1);
    double a[4];
    for ( nat i = 0; i < 4; ++i )
    {
        const Vector3d & v = poly.facet[i].plane.norm;
        double & d = a[i];
        d = v * data[0];
    ...
    const double x1 = ( a[0] + a[1] );
    const double y1 = ( a[0] + a[3] );
    const double z1 = ( a[0] + a[2] );
    const double x2 = -( a[2] + a[3] );
    const double y2 = -( a[1] + a[2] );
    const double z2 = -( a[1] + a[3] );
    poly.vertex[0] = Vector3d ( x1, y2, z1 );
    poly.vertex[1] = Vector3d ( x1, y1, z2 );
    poly.vertex[2] = Vector3d ( x2, y1, z1 );
    poly.vertex[3] = Vector3d ( x2, y2, z2 );
    return true;
}
