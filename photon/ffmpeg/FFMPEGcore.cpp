...
static bool AVFrame2AwpRGBImage(AVFrame* frame, awpImage** ppImage)
{
    ...
    h = sws_scale(context, frame->data, frame->linesize, 0, frame->height, picture.data, picture.linesize);
    if (h != frame->height)
        goto cleanup;
...
static bool AwpRGBImage2AVFrame(awpImage* pImage, AVFrame** ppFrame)
{
    ...
    pFrame = alloc_picture(PIX_FMT_YUV420P, pImage->sSizeX, pImage->sSizeY);
    if (pFrame == NULL)
        goto cleanup;
    ...
    h = sws_scale(context, picture.data, picture.linesize, 0, pFrame->height, pFrame->data, pFrame->linesize);
...
bool CFFMPEGPlayer::Open(const char* lpFileName)
{
    ...
    {
        _OPEN_FILE_ERROR_(FFMPEG_PLAYER_CANNOT_INIT_SDL)
    }
...
int CFFMPEGPlayer::queue_picture(double pts)
{
  ...
  if(vp->allocated && this->m_VideoPicture.GetImage() != NULL)
  {
	...
	SDL_LockMutex(m_pictq_mutex);
	::awpCopyImage(this->m_VideoPicture.GetImage(), &vp->image);
	 SDL_UnlockMutex(m_pictq_mutex);
    vp->pts = pts;
...
void CFFMPEGPlayer::SetUpAudioImage()
{
	...
	float audio_min1 = 10e6;
	float audio_max1 = -10e6;
	float audio_min2 = 10e6;
	float audio_max2 = -10e6;
...
void CFFMPEGPlayer::UpdateAudioImage()
{
	...
	p2.X = p1.X;
	p2.Y = 480 -1;
	awpCopyImage(m_AudioImageBkg, &m_AudioImage);
	awpRGBColor* c = (awpRGBColor*)m_AudioImage->pPixels;
	for (int y = 0; y < 480; y++)
	{
...
bool CFFMPEGPlayer::CImageCompare::InternalCompare(awpImage* pImage, double threshold)
{
        ...
        {
            first_time = false;
            awpCopyImage(pImage, &m_image);
            return false;
        }
        double d = 0;
        AWPBYTE* b1 = (AWPBYTE*)pImage->pPixels;
        AWPBYTE* b2 = (AWPBYTE*)m_image->pPixels;
