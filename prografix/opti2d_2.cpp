...
// Проверка попадания проекции центра круга за пределы отрезока
...
static
bool check ( CArrRef<Edge> edge, CArrRef<nat> support )
{
...
inline
double value ( const Edge & e, CArrRef<Edge> edge )
{
...
Def<Ellipse> maxEllipseInConvexPolygon ( ArrRef<Edge> edge )
{
    ...
    if ( n == 3 ) return maxEllipseIn3gon ( edge[0].line, edge[1].line, edge[2].line );
