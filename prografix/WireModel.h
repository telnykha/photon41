...
WireModel<N> & WireModel<N>::cut ( const Double<N+1> & g, List< Vertex<N> > & stor, WireModel & part )
{
    ...
    do
    {
        ...
        for ( nat16 i = 0; i < N; ++i )
        {
            ...
            nat16 * num2 = vert2->nfacet;
            if ( stor.cur() )
                stor.movCurBefFir ( temp );
            else
                temp.addBefFir ( new Vertex<N> );
            Vertex<N> * vert3 = temp.top();
			vert3->coor = vert2->coor;
			vert3->coor -= vert1->coor;
			vert3->coor *= vert2->data / ( vert1->data - vert2->data );
			vert3->coor += vert2->coor;
    ...
    do
    {
        ...
        if ( stor.cur() )
            stor.movCurBefCur ( part.vlist );
        else
            part.vlist.addBefCur ( new Vertex<N> );
        Vertex<N> * vert2 = part.vlist.cur();
